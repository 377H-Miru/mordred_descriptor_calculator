...
本ツールは、分子記述子計算に Mordred、SMILESのパースおよび分子前処理・座標計算に RDKit を利用しています。
- **Mordred**: BSD-3-Clause ライセンスのもとで配布されています。
- **RDKit**: BSD 系のオープンソースライセンスのもとで配布されています。

依存パッケージ本体の詳細なライセンス条件については、各上流プロジェクト（Mordred / RDKit）を参照してください。
